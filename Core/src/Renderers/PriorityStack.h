...
public:
	...
	const ElementType& Top() const { return mData.front().first; }

	// Gets the priority of the element at the top of the stack
	int TopPriority() const { return mData.front().second; }
...
template <class ElementType, int N = 5, int MAX_PRIORITY_ = 5>
...
public:
	...
	{
		assert(mHighestPriority >= 0);
		return mData[mHighestPriority][mSize[mHighestPriority] - 1];
	}

	// Gets the priority of the element at the top of the stack
	int TopPriority() const { return mHighestPriority; }

	// Gets the second element from the top of the stack
	const ElementType& SecondFromTop() const
	{
		assert(mSecondHighestPriority >= 0);
	...
	void Clear()
	{
		memset(mSize, 0, sizeof(int)*MAX_PRIORITY_);
		mHighestPriority = -1;
		mSecondHighestPriority = -1;
		mTotalSize = 0;
	}
...
private:
	ElementType mData[MAX_PRIORITY_][N]; // Stack data
	int mSize[MAX_PRIORITY_]; // Sizes of each stack
...
public:
	...
	StaticPriorityStack2(const StaticPriorityStack2 & src) : mSize(src.mSize)
	{
		if (mSize > 0)
			::memcpy(mData, src.mData, sizeof(DataElement) * mSize);
	}
	...
	const ElementType& Bottom() const
	{
		assert(mSize > 0);
		return mData[0].element;
	}
	...
	void Push(ElementType element, int priority)
	{
		if (mSize == N) {
			return;
			//LogInformation() << " Stack overflow - clearing last elem";
			//--mSize;
		}
